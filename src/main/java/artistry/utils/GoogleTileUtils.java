 ...
 * These routines are not written for speed. These routines assume a tile size
 * of 256 pixels square. Internally a sort of offset mercator projection is
 * used, this places the origin (0,0) at the top left and goes to +1,+1 at the
 * bottom right. I believe a proper mercator would have 0,0 in the middle and
 * range from -0.5,-0.5 bottom left to +0.5,+0.5 top right. I've used this
...
class GoogleTileUtils {
	...
	public static Rectangle2D.Double getTileRect(int x, int y, int zoom) {
		...
		double bottomLat = Math
				.toDegrees((2 * Math.atan(Math.exp(Math.PI * (1 - (2 * bottomLatMerc))))) - (Math.PI / 2));
	...
	/**
	 * returns the lat/lng as an "Offset Normalized Mercator" pixel coordinate, this
	 * is a coordinate that runs from 0..1 in latitude and longitude with 0,0 being
	 * top left. Normalizing means that this routine can be used at any zoom level
	 * and then multiplied by a power of two to get actual pixel coordinates.
	 * 
	 * @param lat
	 *            in degrees
	 * @param lng
	...
	/**
	 * returns a point that is a google tile reference for the tile containing the
	 * lat/lng and at the zoom level.
	 * 
	 * @param lat
	 * @param lng
	 * @param zoom
	...
	/**
	 * returns a point that is a google pixel reference for the particular lat/lng
	 * and zoom assumes tiles are 256x256.
	 * 
	 * @param lat
	 * @param lng
	 * @param zoom
	...
	/**
	 * Returns a google maps satellite type string for the tile containing the lat
	 * and lng at the zoom level.
	 * 
	 * @param lat
	 * @param lng
	 * @param zoom
